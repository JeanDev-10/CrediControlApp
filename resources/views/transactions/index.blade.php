<x-app-layout>
    ...
    <div class="py-6">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            {{-- Filtros --}}
            <div
                class="bg-white dark:bg-gray-800 shadow sm:rounded-lg p-6 flex flex-col md:flex-row md:justify-between md:items-center gap-4 align-middle">
                <form method="GET" action="{{ route('transactions.index') }}" class="flex flex-wrap gap-4 items-center">
                    <x-text-input name="description" placeholder="Compra de comida"
                        value="{{ request('description') }}" />
                    <x-text-input name="date" type="date" value="{{ request('date') }}" />
                    <select name="type"
                    ...
                    <tbody>
                        @forelse($transactions as $transaction)
                            <tr class="border-b border-gray-200 dark:border-gray-700 text-black dark:text-gray-100">
                                ...
                                @php
                                    if ($transaction->previus_quantity >= 0) {
                                        $color = 'text-green-600 dark:text-green-400';
                                    } else {
                                        $color = 'text-red-600 dark:text-red-400';
                                    }
