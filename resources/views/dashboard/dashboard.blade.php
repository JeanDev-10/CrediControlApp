<x-app-layout>
    ...
    @role("admin")
    <div class="py-6 px-2 sm:px-4 lg:px-8 bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg mt-5">
            <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
                {{ __('Bienvenido admin, puedes administrar') }}
            </h2>
    </div>
    @endrole
    {{-- === CARDS === --}}
        ...
        <x-card-stat class="col-span-2 md:col-span-1" title="Monto Total Pendiente" :value="'$' . number_format($stats['pending_debts_total'], 2)" color="text-green-400">
            <x-slot name="icon">
                <svg class="w-8 h-8 text-green-600 dark:text-green-400" xmlns="http://www.w3.org/2000/svg" fill="none"
                    viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        d="M12 8c-3 0-6 2-6 6 0 3 2.5 6 6 6s6-3 6-6c0-4-3-6-6-6zm0-6v2m0 16v2m4.24-13.76L16 8m-8 0l-.24-.76M4 14h16" />
                </svg>
            </x-slot>
        </x-card-stat>
    </div>
